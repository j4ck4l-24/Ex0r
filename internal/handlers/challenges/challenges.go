...
func GetAllChallenges(c *fiber.Ctx) error {
	...
	if role != "Admin" {
		...
		if err != nil {
			return utils.SendPublicChallengesResponse(c, fiber.StatusInternalServerError, "Failed to fetch challenges", challenges)
		}
		if len(challenges) == 0 {
			return utils.SendPublicChallengesResponse(c, fiber.StatusNotFound, "No challenges found", challenges)
		}
		return utils.SendPublicChallengesResponse(c, fiber.StatusOK, "Challenges Fetched Succesfully", challenges)

	} else {
		challenges, err := services.FetchAdminChallenges(c, dbConn)
		if err != nil {
			return utils.SendAdminChallengesResponse(c, fiber.StatusInternalServerError, "Failed to fetch challenges", challenges)
		}
		if len(challenges) == 0 {
			return utils.SendAdminChallengesResponse(c, fiber.StatusNotFound, "No challenges found", challenges)
		}
		return utils.SendAdminChallengesResponse(c, fiber.StatusOK, "Challenges Fetched Succesfully", challenges)
	}
}
...
func GetChallenge(c *fiber.Ctx) error {
	...
	if role != "Admin" {
		...
		if err != nil {
			return utils.SendGeneralResponse(c, fiber.StatusBadRequest, "Invalid chall_id")
		}
		return utils.SendPublicChallengeResponse(c, fiber.StatusOK, "Challenge fetched Succesfully", challenge)
	} else {
		challenge, err := services.FetchAdminChallenge(c, dbConn)
		if err != nil {
			return utils.SendGeneralResponse(c, fiber.StatusBadRequest, err.Error())
		}
		return utils.SendAdminChallengeResponse(c, fiber.StatusOK, "Challenge fetched Succesfully", challenge)
	}
}

func CreateChallenge(c *fiber.Ctx) error {
	dbConn := db.DB
	challenge := new(models.ChallengeAdmin)
	...
	if challenge.Type == "static" {
		...
		if err != nil {
			return utils.SendGeneralResponse(c, fiber.StatusInternalServerError, "Failed to create challenge")
		}
		return utils.SendGeneralResponse(c, fiber.StatusOK, fmt.Sprintf("Challenge created successfully with id = %d", challenge.ChallId))
	...
	return utils.SendGeneralResponse(c, fiber.StatusBadRequest, "Invalid challenge type")
}
...
func UpdateChallenge(c *fiber.Ctx) error {
	...
	if challenge.Type == "static" {
		...
		if err != nil {
			return utils.SendGeneralResponse(c, fiber.StatusInternalServerError, "Failed to update challenge either there is an error or challenge with chall_id doens't exist")
		}
		return utils.SendGeneralResponse(c, fiber.StatusOK, fmt.Sprintf("Challenge updated successfully with id = %d", challenge.ChallId))
...
func DeleteChallenge(c *fiber.Ctx) error {
	...
	if err != nil {
		return utils.SendGeneralResponse(c, fiber.StatusOK, "Failed to delete challenge")
	}
	return utils.SendGeneralResponse(c, fiber.StatusOK, fmt.Sprintf("Challenge deleted successfully with chall_id = %d", chall_id))
}
